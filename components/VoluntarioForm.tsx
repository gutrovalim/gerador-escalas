...
export function VoluntarioForm({ initial = {}, onSalvo, onCancelar }: Props) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (!papeis.length) { setErro("Selecione ao menos um papel."); return }
    const ok = await confirm({
      title: initial.id ? "Guardar alterações?" : "Criar voluntário?",
      description: initial.id
        ? "As alterações serão gravadas na base de dados."
        : "O novo voluntário será adicionado à lista.",
